...
Replace the raise with `NotTranslatableWarning` + return `None`. The caller (`pipeline_translators/pipeline_translator.py:48`) already threads `None` through as `Pipeline.schedule = None`, which is the correct degraded state: the pipeline converts and deploys without a schedule, and the operator adds one manually.
...
- **INV-4** When `properties.runtimeState == "Started"` **and** `recurrence` is missing/empty, emit a stronger warning: *"ENABLED in ADF but has no recurrence — pipeline will NOT be scheduled in Databricks"*.
...
| EM-1 | `recurrence` key absent | warn + return `None` |
| EM-2 | `recurrence` is `None` | warn + return `None` |
| EM-3 | `recurrence == {}` | warn + return `None` |
| EM-4 | `recurrence` has partial keys (parse returns `None`) | warn "could not be parsed" + return `None` |
| EM-5 | EM-1..4 **and** `runtimeState == "Started"` | stronger "ENABLED in ADF" warning + return `None` |
...
- `test_translate_schedule_trigger_started_empty_recurrence_emits_stronger_warning`
- `test_translate_schedule_trigger_unparseable_recurrence_warns_and_returns_none`
- `test_translate_schedule_trigger_excepts` — narrowed to `properties` absent only (EM-0)
