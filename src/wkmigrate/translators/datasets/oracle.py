"""Translator for Oracle Database datasets.

This module exposes ``translate_oracle_dataset``, which normalises ADF dataset
definitions of type ``OracleTable`` into ``SqlTableDataset`` IR objects.  Translation
resolves the backing Oracle linked-service specification to obtain connection metadata
(host, database, credentials, and authentication type), then extracts the schema and
table identifiers from the dataset properties.  The fully qualified ``schema.table``
string is stored in the ``dbtable`` field for use by JDBC-based connectors.  If the
linked-service definition is absent or cannot be parsed the function returns an
...
from wkmigrate.translators.linked_services import translate_oracle_spec
...
def translate_oracle_dataset(dataset: dict) -> SqlTableDataset | UnsupportedValue:
    ...
    Returns:
        Oracle dataset as a ``SqlTableDataset`` object, or an ``UnsupportedValue``
