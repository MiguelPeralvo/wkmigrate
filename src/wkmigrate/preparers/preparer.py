...
def prepare_workflow(
    ...
    """
    activities = [
        prepare_activity(task, files_to_delta_sinks, credentials_scope, emission_config) for task in pipeline.tasks
    ]
...
def prepare_activity(
    ...
    if isinstance(activity, RunJobActivity):
        return prepare_run_job_activity(
            activity,
            default_files_to_delta_sinks,
            credentials_scope=credentials_scope,
            emission_config=emission_config,
    ...
    if isinstance(activity, LookupActivity):
        return prepare_lookup_activity(
            activity,
            credentials_scope=credentials_scope,
            emission_config=emission_config,
        )
    if isinstance(activity, WebActivity):
        return prepare_web_activity(
            activity,
            credentials_scope=credentials_scope,
            emission_config=emission_config,
        )
